...
namespace RLib.Base
{
    ...
    [Serializable]  
    public class RObservableDictionary<TKey, TValue> :  IObservableDictionary<TKey, TValue>, IDictionary, ICollection, IEnumerable  ,IReadonlyObservableDictionary<TKey, TValue>
    {  

        public bool                IsHaveObservableCollection { get; protected set; }
        public IReadonlyObservableCollection<TValue> ObservableCollection { get { return m_pObservableCollection; }}


        protected RObservableCollection<TValue> m_pObservableCollection =new RObservableCollection<TValue>();
        ...
        public RObservableDictionary(bool haveObservableCollection = false)
        {
            IsHaveObservableCollection = haveObservableCollection;
            _keyedEntryCollection = new KeyedDictionaryEntryCollection<TKey>();  
        }  
  
        public RObservableDictionary(IDictionary<TKey, TValue> dictionary, bool haveObservableCollection = false)  
        {  
            IsHaveObservableCollection = haveObservableCollection;
            _keyedEntryCollection = new KeyedDictionaryEntryCollection<TKey>();  
        ...
        public RObservableDictionary(IEqualityComparer<TKey> comparer, bool haveObservableCollection = false)  
        {  
            IsHaveObservableCollection = haveObservableCollection;
            _keyedEntryCollection = new KeyedDictionaryEntryCollection<TKey>(comparer);  
        }  
  
        public RObservableDictionary(IDictionary<TKey, TValue> dictionary, IEqualityComparer<TKey> comparer, bool haveObservableCollection = false)  
        {  
            IsHaveObservableCollection = haveObservableCollection;
            _keyedEntryCollection = new KeyedDictionaryEntryCollection<TKey>(comparer);  
        ...
        protected RObservableDictionary(SerializationInfo info, StreamingContext context)  
        {  
            _siInfo = info;  
        }  
        ...
        IEnumerable<TKey> IReadOnlyDictionary<TKey, TValue>.Keys { get { return TrueDictionary.Keys;} }
    ...
    IEnumerable<TValue> IReadOnlyDictionary<TKey, TValue>.Values { get { return TrueDictionary.Values; } }
        ...
        protected virtual bool AddEntry(TKey key, TValue value, out DictionaryEntry entry)  // mod by rush 2020/4/13  
        {
            DictionaryEntry en = new DictionaryEntry(key, value);
            _keyedEntryCollection.Add(en);
        ...
        {
            if (IsHaveObservableCollection)
            {
                switch (args.Action)
                {
                    case NotifyCollectionChangedAction.Add:
                    {
                        foreach (KeyValuePair<TKey, TValue> kv in args.NewItems)
                            m_pObservableCollection.Add(kv.Value);
                    }
                        break;
                    case NotifyCollectionChangedAction.Remove:
                    {
                        foreach (KeyValuePair<TKey, TValue> kv in args.OldItems)
                            m_pObservableCollection.Remove(kv.Value);
                    }
                        break;
                    case NotifyCollectionChangedAction.Reset:
                    {
                        m_pObservableCollection.Clear();
                    }
                        break;
                }
            }
        ...
        {
            DictionaryEntry en;
            if (AddEntry(key, value, out en))
            {
                _version++;  
  
                // mod by rush 2020/4/13: 这样写效率极低，数量超1万后查100倍 
                //DictionaryEntry entry;  
                //int index = GetIndexAndEntryForKey(key, out entry);  
                //FireEntryAddedNotifications(entry, index);  

                FireEntryAddedNotifications(en, _keyedEntryCollection.Count-1);  
            }  
        }  
        ...
        {  
            ...
            internal Enumerator(RObservableDictionary<TKey, TValue> dictionary, bool isDictionaryEntryEnumerator)  
            {  
            ...
            private RObservableDictionary<TKey, TValue> _dictionary;  
