...
namespace RLib.Graphics.Shape
{
    ...
    public class Line : Shape, ILine
    {
        ...
        public static double?  GetY(Point p1, Point p2, double x)            // 2点构成的直线，给定一个x，求直线的y  [根据直线的2点式计算]
        {
            if (p1.X.NearlyEqual(p2.X))
            {
                if (p1.X.NearlyEqual(x))
                    return x;
                else
                    return null;
            }
            else if (p1.Y.NearlyEqual(p2.Y))
            {
                return p1.Y;
            }
            else
        ...
        {
            get { return  new Rect(Start, new Size(End.X-Start.X, End.Y-Start.Y)); }
        } // 获取总的BoundingBox

        public bool         IsVerticle
        {
            get { return Start.X.NearlyEqual(End.X); }
        }                             // 是否是垂直线
        ...
        public bool         IsHorizontal
        {
            get
            {
                return Start.Y.NearlyEqual(End.Y);
            }
        } // 是否是横线
    ...
    public class Ellipse:SealingShape, IEllipse                             //圆
    {
        ...
        {
            get { return RadiusX.NearlyEqual(RadiusY); }
        }
