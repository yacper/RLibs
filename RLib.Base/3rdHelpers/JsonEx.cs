...
public class DoubleExConverter : JsonConverter
{
    ...
    public override bool CanConvert(Type objectType) { return objectType.IsDouble(); }
...
public class IntExConverter : JsonConverter
{
    ...
    public override bool CanConvert(Type objectType) { return objectType.IsInt(); }
...
{
    public DateTimeFormatConverter(string format) { DateTimeFormat = format; }
        ...
        JsonSerializer serializer)
    {
        ...
        if (reader.TokenType == JsonToken.Date)
            return type == typeof(DateTimeOffset)
                       ? (!(reader.Value is DateTimeOffset) ? (object)new DateTimeOffset((DateTime)reader.Value) : reader.Value)
                       : (reader.Value is DateTimeOffset dateTimeOffset ? (object)dateTimeOffset.DateTime : reader.Value);
        string str = reader.Value?.ToString();
        if (str.IsNullOrEmpty() & flag)
            return (object)null;
        return type == typeof(DateTimeOffset)
                   ? (!(this.DateTimeFormat.IsNullOrEmpty())
                          ? (object)DateTimeOffset.ParseExact(str, this.DateTimeFormat, (IFormatProvider)this.Culture, this.DateTimeStyles)
                          : (object)DateTimeOffset.Parse(str, (IFormatProvider)this.Culture, this.DateTimeStyles))
                   : (!(this.DateTimeFormat.IsNullOrEmpty())
                          ? (object)DateTime.ParseExact(str, this.DateTimeFormat, (IFormatProvider)this.Culture, this.DateTimeStyles)
                          : (object)DateTime.Parse(str, (IFormatProvider)this.Culture, this.DateTimeStyles));
    }
}
